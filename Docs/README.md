...
ACL tries very hard to be as safe as possible and as such very few things require tuning. Most compression settings exposed are present for debugging purposes and are not meant to be changed in production.

### Skeleton

![Skeleton Compression Settings](Images/CompressionSettings_Skeleton.png)
...
![Skeleton Compression Settings](Images/CompressionSettings_SafetyFallback.png)

Despite the best efforts of ACL, some exotic animation sequences will end up having an unacceptably large error, and when this happens, it will attempt to fall back to safer settings. This should happen extremely rarely if the virtual vertex distances are properly tuned. In order to control this behavior, a boolean flag is provided to enable or disable the fallback logic as well as a threshold to control when it kicks in. As ACL improves over time, the fallback might become obsolete.

### Clip

![Skeleton Compression Settings](Images/CompressionSettings_Clip.png)
...
![Skeleton Compression Settings](Images/CompressionSettings_Segmenting.png)
